...
public class MqmRestClientImpl extends AbstractMqmRestClient implements MqmRestClient {
	...
	private static final String URI_GET_ABRIDGED_TASKS = "analytics/ci/servers/{0}/tasks?self-url={1}&plugin-version={2}&api-version={3}";
	...
	@Override
	public String getAbridgedTasks(String selfIdentity, String selfLocation, String pluginVersion, Integer apiVersion) {
		HttpGet request;
		HttpResponse response = null;
		String responseBody;
		try {
			request = new HttpGet(createSharedSpaceInternalApiUri(URI_GET_ABRIDGED_TASKS, selfIdentity, selfLocation, pluginVersion, apiVersion));
			response = execute(request);
