...
//		serveFile(res, TEST_AUDIT_FILE, Flavor.JSON);
//	}
...
//					.setUrl(OctaneSDK.getClients().get(0).getPluginService().getOctaneConfiguration().getUrl() +
...
//			OctaneResponse response = OctaneSDK.getClients().get(0).getRestService().obtainOctaneRestClient().execute(request);
