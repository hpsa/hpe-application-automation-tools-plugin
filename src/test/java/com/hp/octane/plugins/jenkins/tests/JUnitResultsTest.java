...
public class JUnitResultsTest {
	...
	//	matrixProject.setAxes(new AxisList(new Axis("OS", "Linux", "Windows")));
        matrixProject.setAxes(new AxisList(new Axis("param", "A", "B")));
		...
		Assert.assertEquals(new HashSet<>(Arrays.asList(projectName + "/param=B#1", projectName + "/param=A#1")), getQueuedItems());
